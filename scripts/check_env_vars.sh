...
check_env_vars() {
    local script_file="$1"  # Accept the script name as an argument

    # Initialize an array to hold required variables
    REQUIRED_VARS=()

    if [ -f "$script_file" ]; then
        while IFS= read -r var; do
            REQUIRED_VARS+=("$var")
        done < <(extract_required_vars "$script_file")
    else
    ...
    if [ ${#REQUIRED_VARS[@]} -eq 0 ]; then
        echo "No required environment variables found in the script file '$script_file'."
        exit 1
    fi
    ...
    echo "Required Variables (from script '$script_file'):"
    for VAR in "${REQUIRED_VARS[@]}"; do
        echo "$VAR"
    done
    echo "----------------------------"
...
# Call the function with the script passed as an argument
check_env_vars "$1"
